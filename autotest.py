...
def retrieve_tests(one_mod: str|None = None) -> TestCase:
    if one_mod:
        ...
        return [cls for name, cls in vars(module).items() if 
               ...
               TestCase) and cls is not TestCase]
    return TestLoader().discover(start_dir=str(
           utils.TESTS_DIR), pattern="*.py")

def test_suite(suite: list[type]|None = None) -> TestSuite:
    if suite is None: return retrieve_tests()
    if isinstance(suite, list):
        ts = TestSuite()
        for cls in suite: ts.addTest(TestLoader(
                          ).loadTestsFromTestCase(cls))
        return ts

def render_trace(result: TestResult) -> None:
    hues    = ["red", "red", "yellow"]
    content = {
