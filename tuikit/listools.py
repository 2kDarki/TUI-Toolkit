...
def choose(options: dict, cursor: str = ">>> ",
    ...
    Usage:
      ...
      - The values of options (dict) should be functions. If not,
        use pick_from(...) instead.
      - Write 'clear' to reset the source screen.
      - This function works best when paired with the function
        list_items(...)
    ...
    Args:
        ...
        getch   (bool): If true returns the index position of the
                        chosen function rather than calling it.
                        NB: if needed, you can use this flag for
                        options that are lists or non-callable
                        dictionary values to return their index
                        position then use pick_from(...) to get
                        the value
        src (callable): The name of the function/method where
                        this function was called from. Needed for
                        clearing the screen and recalling where
                        you were
        proxy   (bool): Needed in rare cases where the
                        function/method that called this function
                        was called by another function/method
                        and it needs to know that. See Habitrax
                        for example
    """
    if back:
...
def pick_from(storage: list | tuple | dict, choice: int,
    ...
    Args:
        ...
        index: If None (default), raises StopIteration when the
               provided choice is greater than storage's index
               positions. Otherwise use index as fallback choice
    """
...
def flatten(data: Iterable, to: type = list) -> list | tuple:
    """
    Recursively flattens any iterable (except str/bytes) into a
    flat list or tuple.

    Args:
        data (Iterable): Input iterable, possibly nested.
        to (list or tuple): What to return the flattened data as
...
class ListOrder:
    ...
    def __call__(self, index): return self.format(index)
