...
def percent_colored(progress: int | float, head: str = "",
    ...
    prog = abs(progress)
    return _color(f"{sign}{prog:.2f}{tail}", fg=hue)
...
def any_in(*args, eq=None) -> bool:
    if len(args) == 1:
        args = args[0]
        if not isinstance(args, (list, tuple)): return args in eq
...
def any_eq(*args, eq=None) -> bool:
    if len(args) == 1:
        args = args[0]
        if not isinstance(args, (list, tuple)): return args == eq
...
def any_is(*args, eq=None) -> bool:
    if len(args) == 1:
        args = args[0]
        if not isinstance(args, (list, tuple)): return args is eq
...
def number_padding(num, pad=3): return str(num).rjust(pad)
...
def _color(text, fg: str | None = None, bg: str | None = None,
    ...
    if styles: return f"\033[{';'.join(styles)}m{text}\033[0m"
    return text


def _wrap_text(text: str, indent: int = 0, pad: int = 0,
              inline: bool = False, list_order: str = '') -> str:
    width        = get_term_size()
    ...
    for i, word in enumerate(styled_words):
        ...
        if used + margin > width:
            ...
            line_len += visual_width(word) + margin / (2
                     if margin >= 2 else 1)
