...
class Timezone():
    ...
    def get_offset(self, tz: str | int | float) -> int | float:
        def error():
            raise TimeError(cause=tz,
                  required="IANA timezone (e.g., 'Africa/"
    ...
    def list_zones(self, aliased: bool = False,
        ...
        Args:
            aliased: If True, show alias keys (e.g., 'CAT')
            getter: If True, return list instead of printing
            sort: filter list by continent or abbreviation

        Returns:
            list[str]: Sorted list of zones/aliases
        """
        def error():
            raise InputError(cause=sort,
                             required=f"filter for {chosen}")
    ...
    def localize(self, *args) -> datetime | list[datetime]:
        ...
        def error(dt="hapana"):
            if dt != "hapana":
                raise TimeError(cause=dt,
                      required="iso string (YYYY-MM-DD)"
                    + "or datetime object")
        ...
        def local(dt):
            ...
            return dt.replace(tzinfo=self.zone) if (
                dt.tzinfo is None) else dt.astimezone(self.zone)

        # Handle list/tuple input
        if len(args) == 1 and isinstance(args[0], (list, tuple)):
            return [local(dt) for dt in args[0]]

        # Handle multiple arguments
        elif len(args) > 1: return [local(dt) for dt in args]
