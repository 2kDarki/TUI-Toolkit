...
from tuikit.logictools import shave, format_order
from tuikit.exceptions import warning, TimeError
from tuikit.textools import wrap_text
...
def format_time(time, set_to="hour", hue="", 
                tense=None, precise: bool = True,
                faulty=True):
    """
...
Args:
     ...
     set_to: specify what order the parameter 'time' 
    ...
    precise: whether or not to return the time string
             ...
             then return "55 years, 9 months, 2 weeks, 
             3 days, 14 hours, 37 minutes and 41 
             seconds". Else return "55 years and 9 
             months"
    """
    ...
    if not precise and time > 60:
        return style_text(time_lapsed(time, tense=
               tense), hue)
        
    def get_timeunits(time, units):
        timeunits = []
    ...
    timeunits = get_timeunits(time, units)
    for i, timeunit in enumerate(timeunits):
        time, unit = timeunit
        unit = pluralize(time, unit)
        timeunits[i] = [time, unit]
        
    return style_text(format_time_string(timeunits[
           ::-1], tense=tense), fg=hue)

def convert(time, to="hours"):
    """
...
The time string should be in one of these 3 formats:
    ...
    try:
        parts = [int(part) for part in time.split(
                ":")][:3]
        for part, limit, unit in zip(parts, 
                limits[::-1], units[::-1]):
        ...
        err(time, 
            "time string (hh, hh:mm or hh:mm:ss)")
...
def timestamp(iso_str, date_only=False, 
              short=False, mini=False):
    """
Converts ISO timestamp into a human-readable string.

Args:
    iso_str: ISO 8601 datetime string or object.
...
def time_lapsed(last, now=None, tense="past"):
    """
...
string (e.g., "3 hours and 12 minutes ago").
    ...
    for unit, limit in zip(units[1:], limits[:-1]):
        prv_unit = "second"
        if lapsed >= limit:
            s, lapsed = format_unit(lapsed, limit, 
                unit, prv_unit, tense)
            prv_unit = unit
        else: break
...
def format_time_string(*args, tense=None):
    def tensed(body):
    ...
    parts = []
    try:
        parts = [f"{unit[0]} {unit[1]}" for unit in 
                args if unit[0] != 0]
    except TypeError: 
        if args[0] != 0:parts=[f"{args[0]} {args[1]}"]
    
    if len(parts) > 1: return tensed(', '.join(
        parts[:-1]) + f" and {parts[-1]}")
    elif len(parts) == 1: return tensed(parts[0])
    else: return "just now"

def format_unit(lapsed, unit, major_label, 
                minor_label, tense=None):
    ...
    return format_time_string([major, major_str],
        [minor, minor_str], tense=tense), major
        
def get_time_lapsed(iso_str, fuzzy=True, _fixed=None,
                    num=False):
...
def get_age(b_day, years=True, _fixed=False):
    try:
        b_day = datetime.fromisoformat(b_day)
    except (ValueError, TypeError):
        err(b_day, "iso string (YYYY-MM-DD)")
    
    today = EIGHTEENTH if _fixed else datetime.now()
    if years: return int((today-b_day).days/365)
    lapsed = get_time_lapsed(to_iso(b_day), 0, _fixed)
    return format_time(lapsed, set_to="second", 
           faulty=False)
    
def get_unit_limits(upto: str = "hour", plural=False):
...
def clock(time, notation: str = "24", meridiem: str | None = None) -> str:
    try: time = float(time)
    ...
    for time, (unit, limit) in zip(times, unitlimits):
        if int(time) not in range(limit):
            ...
            raise TimeError(f"{time!r} {xrange}"
                  +f" value allowed for {unit}")
