...
def format_time(time: int | float, set_to: str = "second",
    ...
    Args:
       time: a positive or negative number. If negative, it
             gives out a string like "24 minutes and 30 seconds
             ago". Else it returns "24 minutes and 30 seconds"
     set_to: specify what order the argument 'time' is in.
             <time=1, set_to='hour'> is greater than <time=56,
             set_to='second'>. Default=second
    ...
    precise: whether or not to return the time string down to
             the second. E.g., if precise=True it returns "55
             years, 9 months, 2 weeks, 3 days, 14 hours, 37
             minutes and 41 seconds". Else returns "55 years and
             9 months"
     faulty: used when the time is greater than a month. The
             conversion used are faulty by default (1 month ≠ 4
             weeks) so it is used for corrections. You can leave
             it as-is (it just takes a few milliseconds longer to
             format the time)
    """
    if set_to not in UNITTHRESH: err(set_to, "period of time")

    try: past = time < 0
    except TypeError: err(time, set_to)

    if tense and tense not in ["future", "past"]:
        err(tense, "temporal reference ('future' or 'past')")
    ...
    if not precise and time > 60:
        return style_text(time_lapsed(time, tense=tense), hue)
...
def convert(time: str, to: str = "hours") -> int | float:
    ...
    length = 0
    for i, part in enumerate(parts): length += part / 60**i
...
def timestamp(iso_str: str | datetime | None = None,
    ...
    Args:
        ...
        date_only: If True, omits time (returns date only)
        short: If True, returns compact date (e.g., "14 Sep
               2025").
        mini: If True, returns compact date (e.g., "2025-09-14")
...
def time_lapsed(last: int | float,
...
def get_time_lapsed(iso_str: str, fuzzy: bool = True,
                    num: bool = False,  _fixed: str | None = None
                   ) -> str | int | float:
    ...
    if not num:
        ...
        lapsed = iso_str
        fuzzy  = False
    if years: lapsed -= lapsed - 29_030_400 * years
    return time_lapsed(lapsed) if fuzzy else lapsed


def to_iso(iso_str: str) -> str:
    if isinstance(iso_str, datetime): return iso_str.isoformat()
    try:
