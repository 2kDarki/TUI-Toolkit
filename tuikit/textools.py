...
class Align:
    ...
    def center(self, arg: Any, line: str = " ",
        ...
        if get_pad: return left_pad, total_pad, right_pad
...
def styled(text: str, get: bool = False) -> bool | list:
    ...
    if logictools.any_eq("\033", "\x1b[", eq=text): return []
...
def style_text(text, fg: str = "", bg: str = "",
    ...
    if style: return f"\033[{';'.join(style)}m{clean}\033[0m"
    return clean
...
def flatten(text: str) -> str: return " ".join(text.split())
...
def wrap_text(text: str,  indent: int = 0,  pad: int = 0,
    ...
    if isinstance(text, str):
        if visual_width(text) <= width and not order: return text
...
def label(iterable: list | tuple | dict, hue: str = "cyan",
    ...
    if ordered: return [style_text(f"{i + 1}. {head}", hue)
